...
export default async function GlobalNewQuotationPage() {
  const clients = await prisma.client.findMany({ orderBy: { name: "asc" } });

  return (
    <div className="max-w-4xl space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">New Global Quotation</h1>
        <p className="text-muted-foreground">Create a new quotation not linked to a specific site.</p>
      </div>
      <QuotationForm 
        clients={clients} 
        defaultTerms={DEFAULT_TERMS} 
        defaultExclusions={DEFAULT_EXCLUSIONS}
        action={createIndependentQuotation}
      />
    </div>
  );
}
