...
import { Edit, X, User, Mail, Phone, IndianRupee, MapPin, CreditCard, Building2, Calendar, Landmark, Hash, Lock, ChevronDown } from "lucide-react";
...
export function EditSupervisorForm({ supervisor, allSites = [] }: { supervisor: any; allSites?: SiteOption[] }) {
  ...
  // Reset selected sites when dialog opens
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-0 shadow-2xl z-[101] max-h-[90vh] overflow-y-auto">
          ...
          <form action={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input name="phone" defaultValue={supervisor.phone || ""} placeholder="9876543210" className={inputClass} />
                </div>
                <div className="space-y-1.5">
                  <Label className={labelClass}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input name="aadharNumber" defaultValue={supervisor.aadharNumber || ""} placeholder="1234 5678 9012" className={inputClass} />
                </div>
                <div className="space-y-1.5">
                  <Label className={labelClass}>
                    <IndianRupee className="h-3.5 w-3.5 text-rose-500" /> Monthly Salary
                  </Label>
                  <Input name="monthlySalary" type="number" defaultValue={supervisor.monthlySalary || ""} placeholder="e.g. 30000" className={inputClass} />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input name="accountNumber" defaultValue={supervisor.accountNumber || ""} placeholder="Account No." className={inputClass} />
                </div>
                <div className="space-y-1.5">
                  <Label className={labelClass}>
                    <Hash className="h-3.5 w-3.5 text-indigo-500" /> IFSC Code
                  </Label>
                  <Input name="ifscCode" defaultValue={supervisor.ifscCode || ""} placeholder="e.g. ICIC0001234" className={inputClass} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
            ...
            {allSites.length > 0 && (
              <div className="space-y-4">
                <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-2">
                  <Building2 className="h-4 w-4 text-indigo-500" /> Assigned Sites
                </h3>
                  ...
                  >
                    <span className={selectedSites.length > 0 ? "" : "text-slate-400"}>
                      {selectedSites.length > 0
                        ? `${selectedSites.length} site${selectedSites.length > 1 ? "s" : ""} assigned`
            ...
            <div className="space-y-1.5">
              ...
              <Input name="password" placeholder="Leave blank to keep current" className={inputClass} />
            </div>
