            ...
            <div className="flex flex-wrap gap-2.5 mt-2">
              {assigned.map((a) => (
                <a 
                  ...
                  className={`px-4 py-2 rounded-xl text-sm font-bold transition-all hover:-translate-y-0.5 border ${
                    a.siteId === siteId 
      ...
      <AttendanceForm 
        ...
        headerControls={
          <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
            <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900 rounded-xl p-1.5 border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900 rounded-xl p-1.5 border border-slate-200 dark:border-slate-800 shadow-sm flex-1 md:flex-none">
              <div className="p-2 bg-white dark:bg-slate-800 rounded-lg shadow-sm text-indigo-600 dark:text-indigo-400">
                <Building2 className="h-4 w-4" />
              </div>
              <select name="buildingId" className="h-9 w-full md:w-auto min-w-[200px] rounded-md border-0 bg-transparent px-3 text-sm font-bold focus:ring-0 cursor-pointer outline-none text-slate-700 dark:text-slate-200">
                <option value="" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">All Buildings (General)</option>
                {site.buildings.map((b: any) => <option key={b.id} value={b.id} className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">{b.name}</option>)}
              </select>
            </div>
          </div>
        }
              ...
              <thead className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800 text-slate-500 uppercase text-xs font-bold tracking-wider">
                <tr>
                  <th className="px-6 py-5 w-1/3">
                    <div className="flex items-center gap-2"><Users className="h-4 w-4 text-blue-500" /> Labourer</div>
                  </th>
                  <th className="px-6 py-5 w-[220px]">
                    <div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-500" /> Hajari</div>
                  </th>
                  <th className="px-6 py-5 min-w-[250px]">
                    <div className="flex items-center gap-2"><MessageSquare className="h-4 w-4 text-purple-500" /> Remarks</div>
                  </th>
                </tr>
              </thead>
                ...
                {site.labourCategories.map((cat: any) => {
                  ...
                  return (
                    <React.Fragment key={cat.id}>
                      {/* Category Header */}
                      <tr>
                        <td colSpan={3} className="px-6 py-4 bg-slate-50/50 dark:bg-slate-800/20 border-b border-slate-100 dark:border-slate-800/40">
                          <span className="inline-flex items-center rounded-lg bg-white dark:bg-slate-800 px-3 py-1.5 text-xs font-bold text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 shadow-sm uppercase tracking-wider">
