...
interface LabourStatusDialogProps {
  id: string;
  active: boolean;
  entityName: string;
...
export function LabourStatusDialog({ id, active, entityName, onToggle, size = "md" }: LabourStatusDialogProps) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <div className={`flex items-center gap-${isSmall ? "2" : "3"}`}>
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{newActive ? "Activate" : "Deactivate"} Labourer</DialogTitle>
            <DialogDescription>
              {newActive
          ...
          <div className="grid gap-4 py-4">
            ...
            {!newActive && (
              <div className="grid gap-2">
                ...
                <Textarea
                  id="reason"
                  placeholder="Why is this labourer going inactive?"
                  value={reason}
