...
const BOTTOM_LIMIT = 120; // Safe printable boundary strictly above footer & signature stamp (y=15 to 110)
...
export async function generateBillPdfPackage(data: {
  ...
  page.drawText("Sr.", { x: MARGIN + 5, y: y - 13, size: 8.5, font: bold, color: black });
  page.drawText("Particulars / Work Description", { x: MARGIN + 35, y: y - 13, size: 8.5, font: bold, color: black });
  page.drawText("Amount (Rs.)", { x: PAGE_W - MARGIN - 90, y: y - 13, size: 8.5, font: bold, color: black });
  y -= 18;
  ...
  if (supplyTotal > 0) {
    page.drawText(String(sr++), { x: MARGIN + 5, y: y - 13, size: 8.5, font, color: darkGray });
    page.drawText(`Departmental Extra Labour Supply (Fitters & Helpers Log Billed)`, { x: MARGIN + 35, y: y - 13, size: 8.5, font, color: black });
    page.drawText(formatCurrency(supplyTotal), { x: PAGE_W - MARGIN - 90, y: y - 13, size: 8.5, font, color: black });
    y -= 16;
  }
  ...
  page.drawText("Taxable Work Done Amount:", { x: MARGIN + 180, y, size: 9, font: bold, color: black });
  page.drawText(formatCurrency(currentTotal), { x: PAGE_W - MARGIN - 90, y, size: 9, font: bold, color: black });
  y -= 14;

  page.drawText(`Add CGST @ ${cgstPct}%:`, { x: MARGIN + 180, y, size: 8.5, font, color: darkGray });
  page.drawText(formatCurrency(cgstAmt), { x: PAGE_W - MARGIN - 90, y, size: 8.5, font, color: darkGray });
  y -= 13;

  page.drawText(`Add SGST @ ${sgstPct}%:`, { x: MARGIN + 180, y, size: 8.5, font, color: darkGray });
  page.drawText(formatCurrency(sgstAmt), { x: PAGE_W - MARGIN - 90, y, size: 8.5, font, color: darkGray });
  y -= 15;
  ...
  page.drawRectangle({ x: MARGIN, y: y - 50, width: PAGE_W - 2 * MARGIN, height: 50, color: lightGray, borderColor: teal, borderWidth: 1 });
  page.drawText("BANK DETAILS FOR PAYMENT (NEFT / RTGS)", { x: MARGIN + 10, y: y - 13, size: 8.5, font: bold, color: teal });
  page.drawText("ACCOUNT NAME: RCR ENTERPRISES", { x: MARGIN + 10, y: y - 26, size: 8, font: bold, color: black });
  page.drawText("ACCOUNT NO: 088405500559", { x: MARGIN + 10, y: y - 38, size: 8, font, color: darkGray });
  page.drawText("IFSC CODE: ICIC0000884", { x: MARGIN + 230, y: y - 26, size: 8, font, color: darkGray });
  page.drawText("BANK NAME: ICICI BANK LTD.", { x: MARGIN + 230, y: y - 38, size: 8, font, color: darkGray });
  ...
  const drawAbstractHeader = () => {
    page.drawRectangle({ x: MARGIN, y: y - 18, width: PAGE_W - 2 * MARGIN, height: 18, color: lightGray });
    page.drawText("Sr.", { x: MARGIN + 4, y: y - 13, size: 7.5, font: bold, color: black });
    page.drawText("Description", { x: MARGIN + 22, y: y - 13, size: 7.5, font: bold, color: black });
    page.drawText("Unit", { x: MARGIN + 175, y: y - 13, size: 7.5, font: bold, color: black });
    page.drawText("W.O. Area", { x: MARGIN + 202, y: y - 13, size: 7.5, font: bold, color: black });
    page.drawText("Rate", { x: MARGIN + 252, y: y - 13, size: 7.5, font: bold, color: black });
    page.drawText("Prev. Amt", { x: MARGIN + 295, y: y - 13, size: 7.5, font: bold, color: black });
    page.drawText("This Bill Amt", { x: MARGIN + 365, y: y - 13, size: 7.5, font: bold, color: black });
    page.drawText("Cum. Amt", { x: PAGE_W - MARGIN - 75, y: y - 13, size: 7.5, font: bold, color: black });
    y -= 18;
  };
  ...
  towers.forEach((b: any, idx: number) => {
    newPageIfNeeded(25, "SECTION 2: ABSTRACT SUMMARY", drawAbstractHeader);
  ...
  page.drawRectangle({ x: MARGIN + 160, y: y - 15, width: PAGE_W - MARGIN - 160 - MARGIN, height: 16, color: lightGray });
  page.drawText("NET PAYABLE INVOICE BALANCE:", { x: MARGIN + 170, y: y - 11, size: 8.5, font: bold, color: teal });
  page.drawText(formatCurrency(netBalAmt), { x: PAGE_W - MARGIN - 90, y: y - 11, size: 8.5, font: bold, color: teal });
  y -= 18;

  if (grossContractValue > 0) {
    page.drawText(`GROSS CONTRACT VALUE (ALL TOWERS):`, { x: MARGIN + 170, y, size: 7.5, font: bold, color: darkGray });
    page.drawText(formatCurrency(grossContractValue), { x: PAGE_W - MARGIN - 90, y, size: 7.5, font: bold, color: darkGray });
  }
  ...
  for (const tower of towers) {
    ...
    page.drawText(`BUA Area: ${approxArea.toLocaleString()} Sft  @  Rs. ${contractRate}/Sft  =  ${formatCurrency(totalTowerVal)}`, {
      x: MARGIN, y, size: 8.5, font: bold, color: teal
    });
    y -= 16;

    const drawTowerHeader = () => {
      page.drawRectangle({ x: MARGIN, y: y - 16, width: PAGE_W - 2 * MARGIN, height: 16, color: lightGray });
      page.drawText("Sr.", { x: MARGIN + 4, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Particulars of Item", { x: MARGIN + 20, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Unit", { x: MARGIN + 168, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Prev %", { x: MARGIN + 192, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Curr %", { x: MARGIN + 230, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Cum %", { x: MARGIN + 268, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Prev Amt", { x: MARGIN + 305, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Curr Amt", { x: MARGIN + 375, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Cum Amt", { x: PAGE_W - MARGIN - 65, y: y - 12, size: 7, font: bold, color: black });
      y -= 16;
    };
    ...
    items.forEach((item: any, i: number) => {
      newPageIfNeeded(20, `BUILDING - ${tower.name.toUpperCase()}`, drawTowerHeader);
      ...
      page.drawText(String(i + 1), { x: MARGIN + 4, y: y - 10, size: 7, font, color: darkGray });
      page.drawText((item.name || "").slice(0, 28), { x: MARGIN + 20, y: y - 10, size: 7, font, color: black });
      page.drawText(item.unit || "%", { x: MARGIN + 168, y: y - 10, size: 7, font, color: darkGray });
      page.drawText(`${prevQ}%`, { x: MARGIN + 192, y: y - 10, size: 7, font, color: darkGray });
      page.drawText(`${currQ}%`, { x: MARGIN + 230, y: y - 10, size: 7, font: bold, color: teal });
      page.drawText(`${cumQ}%`, { x: MARGIN + 268, y: y - 10, size: 7, font: bold, color: black });

      page.drawText(formatCurrency(prevA), { x: MARGIN + 305, y: y - 10, size: 7, font, color: darkGray });
      page.drawText(formatCurrency(currA), { x: MARGIN + 375, y: y - 10, size: 7, font: bold, color: teal });
      page.drawText(formatCurrency(cumA), { x: PAGE_W - MARGIN - 65, y: y - 10, size: 7, font: bold, color: black });
      y -= 13;
    });

    newPageIfNeeded(25, `BUILDING - ${tower.name.toUpperCase()}`, drawTowerHeader);
    y -= 4;
    page.drawLine({ start: { x: MARGIN, y }, end: { x: PAGE_W - MARGIN, y }, thickness: 1, color: teal });
    y -= 12;
    page.drawText(`TOTAL ${tower.name.toUpperCase()} WORK DONE:`, { x: MARGIN + 150, y, size: 7.5, font: bold, color: black });
    page.drawText(formatCurrency(tPrevTotal), { x: MARGIN + 305, y, size: 7.5, font: bold, color: darkGray });
    page.drawText(formatCurrency(tCurrTotal), { x: MARGIN + 375, y, size: 7.5, font: bold, color: teal });
    page.drawText(formatCurrency(tCumTotal), { x: PAGE_W - MARGIN - 65, y, size: 7.5, font: bold, color: black });
  }
  ...
  if (supplyEntries.length > 0) {
    ...
    const drawSupplyHeader = () => {
      page.drawRectangle({ x: MARGIN, y: y - 16, width: PAGE_W - 2 * MARGIN, height: 16, color: lightGray });
      page.drawText("Date", { x: MARGIN + 4, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Challan", { x: MARGIN + 55, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Description", { x: MARGIN + 105, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Fitter", { x: MARGIN + 245, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Hrs", { x: MARGIN + 278, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Tot F.Hrs", { x: MARGIN + 305, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Helper", { x: MARGIN + 352, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Hrs", { x: MARGIN + 385, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Tot H.Hrs", { x: MARGIN + 412, y: y - 12, size: 7, font: bold, color: black });
      page.drawText("Amount (Rs)", { x: PAGE_W - MARGIN - 55, y: y - 12, size: 7, font: bold, color: black });
      y -= 16;
    };
    ...
    supplyEntries.forEach((se: any) => {
      newPageIfNeeded(20, "EXTRA SUPPLY LABOUR SHEET", drawSupplyHeader);
      ...
      page.drawText(dateStr, { x: MARGIN + 4, y: y - 10, size: 7, font, color: darkGray });
      page.drawText(se.challanNo || "-", { x: MARGIN + 55, y: y - 10, size: 7, font, color: darkGray });
      page.drawText((se.description || "").slice(0, 24), { x: MARGIN + 105, y: y - 10, size: 7, font, color: black });
      page.drawText(String(se.fitterQty || 0), { x: MARGIN + 245, y: y - 10, size: 7, font, color: darkGray });
      page.drawText(`${se.fitterHours || 8}h`, { x: MARGIN + 278, y: y - 10, size: 7, font, color: darkGray });
      page.drawText(`${fHrs}h`, { x: MARGIN + 305, y: y - 10, size: 7, font: bold, color: teal });
      page.drawText(String(se.helperQty || 0), { x: MARGIN + 352, y: y - 10, size: 7, font, color: darkGray });
      page.drawText(`${se.helperHours || 8}h`, { x: MARGIN + 385, y: y - 10, size: 7, font, color: darkGray });
      page.drawText(`${hHrs}h`, { x: MARGIN + 412, y: y - 10, size: 7, font: bold, color: teal });
      page.drawText(formatCurrency(se.totalAmount || 0), { x: PAGE_W - MARGIN - 55, y: y - 10, size: 7, font: bold, color: black });
      y -= 13;
    });

    newPageIfNeeded(45, "EXTRA SUPPLY LABOUR SHEET", drawSupplyHeader);
    y -= 4;
    page.drawLine({ start: { x: MARGIN, y }, end: { x: PAGE_W - MARGIN, y }, thickness: 1, color: teal });
    y -= 12;
    ...
    page.drawText(`Total Hours: ${sumFitterHrs} Hrs (Fitter)  |  ${sumHelperHrs} Hrs (Helper)`, { x: MARGIN + 105, y, size: 7.5, font: bold, color: darkGray });
    y -= 12;
    page.drawText(`Total Days: ${fDays} Nos (Fitter @ Rs.1,100)  |  ${hDays} Nos (Helper @ Rs.800)`, { x: MARGIN + 105, y, size: 7.5, font: bold, color: darkGray });
    y -= 14;

    page.drawText("TOTAL EXTRA LABOUR SUPPLY AMOUNT:", { x: MARGIN + 105, y, size: 8, font: bold, color: black });
    page.drawText(formatCurrency(totSupplyAmt), { x: PAGE_W - MARGIN - 65, y, size: 8, font: bold, color: teal });
  }
