...
export async function generateSupervisorAttendancePdf(attendances: any[], siteName: string, periodStr: string): Promise<Uint8Array> {
  ...
  const primary = rgb(0.3, 0.2, 0.8); // Indigo-ish
  ...
  y -= 25;
  page.drawText(`Supervisor Attendance Report - ${siteName}`, { x: textStartX, y, size: 14, font: bold, color: primary });
  y -= 16;
  page.drawText(`Period: ${periodStr}`, { x: textStartX, y, size: 10, font, color: darkGray });
  ...
  const cols = [
    { name: "Date", w: 80 },
    { name: "Supervisor Name", w: 150 },
    { name: "Email", w: 200 },
    { name: "Status", w: 80 },
    { name: "Daily Rate", w: 80 },
    { name: "Total Earned", w: 80 },
    { name: "Remarks", w: PAGE_W - 2 * MARGIN - 670 },
  ];
  ...
  attendances.forEach((a, i) => {
    ...
    const remarksLines = wrapText(a.remarks || "", font, 8, cols[6].w - 10);
    const nameLines = wrapText(a.supervisor.name, bold, 9, cols[1].w - 10);
    ...
    // Name
    nameLines.forEach((l, li) => page.drawText(l, { x: colX[1] + 5, y: y - 14 - li * 12, size: 9, font: bold, color: primary }));
    
    // Email
    page.drawText(a.supervisor.email.substring(0, 30), { x: colX[2] + 5, y: y - 14, size: 9, font, color: darkGray });
    
    // Status
    let statusColor = black;
    let statusText = "Absent";
    if (a.status === "PRESENT") {
        statusColor = rgb(0.1, 0.6, 0.2);
        statusText = "Present";
    } else if (a.status === "HALF_DAY") {
        statusColor = rgb(0.8, 0.5, 0.1);
        statusText = "Half Day";
    } else {
        statusColor = rgb(0.8, 0.1, 0.1);
    }

    page.drawText(statusText, { x: colX[3] + 5, y: y - 14, size: 9, font: bold, color: statusColor });
    
    // Rate
    if (a.dailyRate > 0) {
      page.drawText(`Rs ${a.dailyRate}`, { x: colX[4] + 5, y: y - 14, size: 9, font, color: black });
    } else {
      page.drawText(`-`, { x: colX[4] + 5, y: y - 14, size: 9, font, color: darkGray });
    }

    // Earned
    if (a.earnedAmount > 0) {
      page.drawText(`Rs ${a.earnedAmount}`, { x: colX[5] + 5, y: y - 14, size: 9, font: bold, color: black });
    } else {
      page.drawText(`-`, { x: colX[5] + 5, y: y - 14, size: 9, font, color: darkGray });
    }

    // Remarks
    remarksLines.forEach((l, li) => page.drawText(l, { x: colX[6] + 5, y: y - 14 - li * 12, size: 8, font, color: darkGray }));
